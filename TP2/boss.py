...
class Boss(Enemy):
    ...
    def shoot(self, ship):
        leftSpread = ship.x - ship.size * 1.5
        middle = ship.x
        rightSpread = ship.x + ship.size * 1.5
        return [eBullet(leftSpread, ship.y, self), eBullet(middle, ship.y, self),\
                eBullet(rightSpread, ship.y, self)]
